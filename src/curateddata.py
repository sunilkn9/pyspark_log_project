...
class Started:
    ...
    def data_from_s3_curated(self):
        ...
        df = spark.read.csv("C:\\Users\\Sunil Kumar\\Downloads\\cleansed_log_data_details.csv", header=True, inferSchema=True)
        df.show()
       ...
       #log_per_device.write.csv("",header = True)
        ...
        curated.coalesce(1).write.format("snowflake").options(**sfOptions).option("dbtable",
                                                                               "{}".format(
                                                                                   r"curated_log_details")).mode(
                "overwrite").options(header=True).save()
        log_per_device.coalesce(1).write.format("snowflake").options(**sfOptions).option("dbtable",
                                                                      "{}".format(
                                                                          r"log_per_device")).mode(
            "overwrite").options(header=True).save()

        log_across_device.coalesce(1).write.format("snowflake").options(**sfOptions).option("dbtable",
                                                                      "{}".format(
                                                                          r"log_across_device")).mode(
            "overwrite").options(header=True).save()
